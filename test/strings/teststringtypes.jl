...
function Base.next(sp::Base.StringLIPairs{CharString}, i::Int)
...
function Base.leadindpairs(rs::RopeString, idx::Int)
...
Base.done(rs::Base.StringLIPairs{RopeString}, (linear, element, ni)::Tuple{Int, Int, Int}) = element > length(rs.data.strs)
function Base.next(rs::Base.StringLIPairs{RopeString}, (linear, element, i)::Tuple{Int, Int, Int})
    el = rs.data.strs[element]
...
Base.leadindpairs(rs::DecodeString, idx::Int) = Base.leadindpairs(rs.s, idx)
Base.start(sp::Base.StringLIPairs{<:DecodeString}) = start(Base.StringLIPairs(sp.data.s))
Base.done(sp::Base.StringLIPairs{<:DecodeString}, state::Int) = done(Base.StringLIPairs(sp.data.s), state::Int)
